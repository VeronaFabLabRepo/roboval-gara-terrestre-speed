...
#define correction_speed    3 // velocita' di correzione traiettoria rettilinea (0-100)//quella con cui si muova da destra a sinistra

#define turn_speed          25 // velocita' di curvatura (0-100)//quella con cui fa la curve 
...
void loop(){
  ...
  if(!leftFarReading && !rightFarReading && (leftCenterReading || rightCenterReading) ){  // 0x1xx0 || 0xx1x0
    ...
    leftHandWall(); //gira su sË stesso
...
//___________funzione che mette in gioco i sensori usando quelle dopo



void leftHandWall(){

  if(leftCenterReading){ // 1xxxxx

    turnLeft();

  }

  if(!leftCenterReading && rightCenterReading){  // 0xxxx1
...
//_ __________funzioni gira senza uso sensori solo motori
...
void turnLeft(){
  ...
  while(!leftNearReading || !rightNearReading){
    ...
    set_motor(left_motor,(turn_speed-26));
...
void turnRight(){
  ...
  while(!leftNearReading || !rightNearReading){
    ...
    set_motor(right_motor,(turn_speed-26));
...
void set_motor(boolean motor, char speed) { // imposta la velocit‡ tra -100 (indietro) e +100 (avanti)
...
void motor_speed(boolean motor, boolean direction, byte speed) { // imposta la velocit‡ tra 0 e 255
