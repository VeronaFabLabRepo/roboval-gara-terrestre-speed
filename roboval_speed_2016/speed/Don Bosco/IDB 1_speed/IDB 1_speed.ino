...
/*
* @author: Pelleggrino,Rotaru,Orzumati,Condemi,Geniale
 * Robot:  IDB1
 *
 * Roboval \
...
#define full_speed          50 // velocita' massima (0-100)                                65
#define correction_speed_1   6 // velocita' di correzione traiettoria rettilinea (0-100)   6
#define correction_speed_2  15 // velocita' di correzione traiettoria rettilinea (0-100)   6
#define turn_speed          21 // velocita' di curvatura (0-100)                          21
#define debugPeriod        200 // ogni quanti millisecondi inviare i messaggi di debug    700   
...
void straight(){
  if(!leftNearReading){// 0x01x0 il robot e' troppo a sinistra, deve andare piu' a destra quindi alzo il motore sinistro
    ...
    set_motor(right_motor, correction_speed_1);
    return;
  }
  else if(!rightNearReading){// 0x10x0 il robot e' troppo a destra, deve andare piu' a sinistra quindi alzo il motore destro
    set_motor(left_motor, correction_speed_1);
    set_motor(right_motor, full_speed);
    return;
  }

  else{ // 0x11x0 il robot e' sopra la linea
    set_motor(right_motor, full_speed); // ATTENZIONE!!! modificato con il +10
    set_motor(left_motor, full_speed); // idem come sopra
  }
}

void leftHandWall(){
  if((!rightFarReading && leftFarReading) && leftNearReading){  //11xxxx
    turnLeft();
  }
